...
function getDprSafe() {
  try {
    if (typeof wx.getWindowInfo === 'function') {
      ...
      if (win && win.pixelRatio) return Math.min(win.pixelRatio, 3);
    }
    if (typeof wx.getDeviceInfo === 'function') {
      const device = wx.getDeviceInfo();
      if (device && device.pixelRatio) return Math.min(device.pixelRatio, 3);
    }
  } catch (e) {
    // ignore
  }
...
Page({
  ...
  showAllRecords() {
    wx.navigateTo({
      url: '/pages/checkin/records/records'
    });
  },
  ...
  goWeeklyReview() {
    wx.navigateTo({
      url: '/pages/checkin/review/review?mode=week'
    });
  },

  goMonthlyReview() {
    wx.navigateTo({
      url: '/pages/checkin/review/review?mode=month'
    });
  },
  ...
  drawSharePosterV2(ctx, { width, height, shareDate, userInfo, shareStats, xchImg, ghImg }) {
    ...
    ctx.setFontSize(11);
    ctx.setTextAlign('center');
    ctx.fillText('古琴助手 · 古琴习练助手', width / 2, height - 18);
  },
